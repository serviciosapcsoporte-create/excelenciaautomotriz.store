...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (canvas && list) {
    ...
    function estimateFor(l) {
      const dx = l.x - PUENTE.x;
      const dy = l.y - PUENTE.y;
      const dist = Math.sqrt(dx*dx + dy*dy);
      const base = 18 + dist * 0.42;
      const peak = isPeakHour();
      let low = Math.round(base);
      let high = Math.round(base + 13);
      if (peak) { low = Math.round(low * 1.3); high = Math.round(high * 1.3); }
      low = Math.max(15, Math.min(55, low));
      high = Math.max(low + 8, Math.min(65, high));
      return { low, high, peak, dist };
    }
    function createTooltipContent(l, est) {
      const trafficClass = est.peak ? 'heavy' : 'fluid';
      const trafficText = est.peak ? '🔴 Tráfico Pesado en la zona' : '🟢 Tráfico Fluido';
      const wrap = document.createElement('div');
      wrap.innerHTML = `
        <div class="tt-origin">Despacho desde Central Puente Aranda</div>
        <div class="tt-name">${l.nombre} — Willard ${l.ref}</div>
        <div class="tt-time">⏱️ ~${est.low} - ${est.high} min</div>
        <div class="tt-traffic ${trafficClass}">${trafficText}</div>
        <a class="tt-cta" href="${waLink(`Hola, estoy en ${l.nombre}. Vi que el tiempo estimado desde Puente Aranda es de ~${est.low}-${est.high} min. ¿Me confirman disponibilidad de motorizado?`)}" target="_blank" rel="noopener">Pedir domicilio a ${l.nombre}</a>
      `;
      return wrap;
    }
    ...
    function showTip(l, pin) {
      ...
      tip.className = 'map-tooltip neon';
      const content = createTooltipContent(l, est);
    ...
    function select(id) {
      ...
      tooltip.className = 'map-tooltip neon';
      const content = createTooltipContent(l, est);
      tooltip.appendChild(content);
      canvas.appendChild(tooltip);
      const pr = pin ? pin.getBoundingClientRect() : { left: 0, top: 0, width: 0 };
      const cr = canvas.getBoundingClientRect();
      tooltip.style.left = (pr.left - cr.left + pr.width / 2) + 'px';
      tooltip.style.top = (pr.top - cr.top) + 'px';
      requestAnimationFrame(() => tooltip.classList.add('is-visible'));
      let timer = setTimeout(() => tooltip.remove(), 6000);
      tooltip.addEventListener('mouseenter', () => clearTimeout(timer));
      tooltip.addEventListener('mouseleave', () => { timer = setTimeout(() => tooltip.remove(), 1500); });
    }
